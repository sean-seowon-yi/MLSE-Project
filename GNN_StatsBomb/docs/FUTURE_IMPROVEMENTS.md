...
**Improvement (original ask):**

- **Counterfactual-safe encoding:** When extracting \(h_{\text{event}}\) for Phase 7 / policy diagnostics, avoid the situation vector already encoding *who* acted, or swapping \(z_p\) is not a clean counterfactual.
- Options discussed included: drop or mask `acts_in` / `performed_by`, or stop-gradient / zero actor features on the actor→event path.

**What is implemented:** `PlayerSimilarityModel.encode_possession_counterfactual` in `src/phase4_model/model.py` **drops only** `(player, acts_in, event)` (and matching `edge_attr` keys). It **keeps** `(event, performed_by, player)` plus temporal and `context_for*` edges so player nodes and 360 context still participate in message passing. Used by:

- `src/phase7_analysis/situation_comparison.py` (`compare`, `compare_possession`),
- `src/phase6_inference/policy_diagnostic.py`,
- `src/phase7_analysis/possession_animation.py` (via `SituationComparator`).

**Training vs analysis:** **Training** and the default **`encode_possession`** path still use the **full** graph including `acts_in` — that is intentional for masked imitation (and optional `acts_in_dropout` during training). The counterfactual-safe encoder applies only where we substitute \(z_p\) while claiming “the same situation.”

**Residual / optional future work:** Because `performed_by` and the actor’s player node remain in the graph, **multi-hop** paths could still carry some actor-related signal into \(h_{\text{event}}\) in principle. Stricter variants (e.g. also altering `performed_by`, actor-specific stop-gradient, or a dedicated actor-free encoder) are **not** implemented and remain optional hardening if provably actor-agnostic \(h_{\text{event}}\) is required.

**Empirical note:** [EVALUATION_RESULTS.md](EVALUATION_RESULTS.md) §4 reports policy-diagnostic metrics (Spearman ρ, substitute-quality ratio, etc.) from runs where `policy_diagnostic.py` already calls `encode_possession_counterfactual` (`_encode_situations`). **§7 of that file is qualitative neighbor inspection — not an encoder ablation.** The repository does not commit a before/after table comparing full `encode_possession` vs counterfactual-safe encoding; to quantify that difference, run `policy_diagnostic` twice with the two encoders (or compare against archived outputs if you store them).
...
| 8   | **Implemented**           | **Counterfactual path fixed in code:** Phase 7, policy diagnostic, and possession animation use `encode_possession_counterfactual`, which **drops `acts_in`** so actor identity is not messaged player→event into \(h_{\text{event}}\). **Training** still uses full `encode_possession` with `acts_in` by design. **`performed_by` is retained** for connectivity; stricter actor-agnostic encoding is optional future work (see §8 above). *This row previously contradicted the codebase and was corrected to match `situation_comparison.py`.*                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
**Summary:** No item is unnecessary or out of scope. Substantive doc corrections: **#7** — the graph is not “silently wrong” (context geometry is per-event via many context nodes). **#8** — the assessment table once claimed Phase 7 used the full graph for \(h_{\text{event}}\); the code uses `encode_possession_counterfactual` for counterfactual workflows (see §8 and `docs/PHASE7.md`). **#14** was a critical training bug discovered post-hoc and has been fixed. Position ablation, EMA alignment, and acts-in dropout variants are available as architectural options; see `PIPELINE_REGISTRY` in `main.py` (currently **11** named pipelines).
