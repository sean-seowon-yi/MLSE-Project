"""
Feature projection modules for event and player nodes.

Before feeding into the GNN, the raw features of each node type must be
projected into a common latent space of dimension *d*.

- **EventProjection**: 126-D sparse/mixed vector → d via MLP + LayerNorm.
- **PlayerProjection**: (position_idx, team_flag, dx, dy) → d via
  learned position embedding + MLP + LayerNorm.

When ``config.ablate_position`` is True, **PlayerProjection** skips the
position embedding entirely: only the continuous features
(is_possession_team, dx, dy) are projected.  The position_idx column
remains in the graph data for contrastive-loss hard-negative mining but
does not enter the learned representation.
"""

import torch
import torch.nn as nn

from ..config import ModelConfig


class EventProjection(nn.Module):
    """Project Phase 1 event features (126-D) to latent dim *d*."""

    def __init__(self, config: ModelConfig):
        super().__init__()
        d = config.latent_dim
        input_dim = getattr(config, "event_feature_dim", 126)
        self.net = nn.Sequential(
            nn.Linear(input_dim, d),
            nn.ReLU(),
            nn.Linear(d, d),
            nn.LayerNorm(d),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """x: (*, input_dim) → (*, d)"""
        return self.net(x)


class PlayerProjection(nn.Module):
    """
    Project raw player-node features to latent dim *d*.

    Raw features per player node (from Phase 3 graph builder):
      [position_idx (float), is_possession_team (0/1), dx, dy]

    When ``ablate_position`` is False (default), position_idx is used as a
    lookup into a learned embedding table; the result is concatenated with
    the continuous features and passed through a small MLP.

    When ``ablate_position`` is True, position_idx is ignored and only the
    continuous features (is_possession_team, dx, dy) are projected.
    """

    def __init__(self, config: ModelConfig):
        super().__init__()
        d = config.latent_dim
        self.ablate_position = getattr(config, "ablate_position", False)

        if self.ablate_position:
            self.pos_embed = None
            self.net = nn.Sequential(
                nn.Linear(3, d),
                nn.ReLU(),
                nn.Linear(d, d),
                nn.LayerNorm(d),
            )
        else:
            d_pos = config.position_embed_dim
            n_pos = config.n_positions
            self.pos_embed = nn.Embedding(n_pos, d_pos)
            self.net = nn.Sequential(
                nn.Linear(d_pos + 3, d),
                nn.ReLU(),
                nn.Linear(d, d),
                nn.LayerNorm(d),
            )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (N, 4) with columns [position_idx, team_flag, dx, dy].
        Returns (N, d).
        """
        cont = x[:, 1:]                               # (N, 3)
        if self.ablate_position:
            return self.net(cont)
        pos_idx = x[:, 0].long().clamp(0, self.pos_embed.num_embeddings - 1)
        pos_emb = self.pos_embed(pos_idx)              # (N, d_pos)
        h = torch.cat([pos_emb, cont], dim=-1)         # (N, d_pos + 3)
        return self.net(h)
