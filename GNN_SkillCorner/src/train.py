...
from .model import PlayerSimilarityAutoencoder, PositionReconstructionLoss, EmbeddingVarianceLoss
...
class Trainer:
    ...
    def __init__(
        ...
        self.variance_loss = EmbeddingVarianceLoss(target_std=1.0)
        self.variance_weight = config.variance_loss_weight
    ...
    def train_epoch(self) -> float:
        ...
        for batch in pbar:
            ...
            # Reconstruction loss
            recon_loss = self.criterion(predictions, targets)
            
            # Variance regularizer (prevents embedding collapse)
            var_loss = self.variance_loss(embeddings, data.batch)
            
            loss = recon_loss + self.variance_weight * var_loss
        ...
        n = max(num_batches, 1)
        self.history['train_recon_loss'].append(total_recon / n)
        self.history['train_var_loss'].append(total_var / n)
