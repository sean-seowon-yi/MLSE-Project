...
node_features = [
    ...
    dist_to_ball,     # Normalized distance to ball (tactical)
    dist_to_own_goal, # Normalized distance to own goal (role signal)
    dist_to_opp_goal  # Normalized distance to opponent goal (role signal)
]
```

**Node feature matrix shape**: `[22, 9]`

**Example node features:**
```python
# Node 0: Left Back (teammate, not sprinting)
[0.15, -0.30, 2.5, -1.2, 0.0, 0.0, 0.4, 0.6, 0.3]

# Node 11: Opponent Right Winger (opponent, sprinting)
[-0.20, 0.25, -3.1, 1.8, 1.0, 1.0, 0.2, 0.8, 0.2]
```
...
Data(
    x=torch.tensor([22, 9]),           # Node features
...
│  └──────────────────────────────────┘    │
│              ↓               ↓ (skip)    │
│        GATv2 output    +   skip proj     │
│              ↓                            │
│    Output: [22, 32] "Player DNA"         │
│    (no final LayerNorm — preserves       │
│     angular diversity for cosine sim)    │
└──────────────────────────────────────────┘
         ↓
┌──────────────────────────────────────────┐
...
**Why skip connection + no final LayerNorm:**
In a fully-connected graph with 2 GATv2 layers, "over-smoothing" causes all 22 node embeddings to converge to nearly the same vector. The skip connection injects each node's projected raw features directly into the final output, preserving per-node identity. Removing the final LayerNorm keeps the angular diversity that cosine similarity needs to distinguish players.
...
```python
self.gat1 = GATv2Conv(
    in_channels=9,        # Input: node features (incl. ball/goal distances)
...
def create_masked_graph(graph: GraphSnapshot, mask_idx: int):
    ...
    # Mask tactical features that leak position (9-feature setup)
    if data.x.size(1) >= 9:
        data.x[mask_idx, 6] = 0.0  # dist_to_ball
        data.x[mask_idx, 7] = 0.0  # dist_to_own_goal
        data.x[mask_idx, 8] = 0.0  # dist_to_opp_goal
...
**Loss Function:**
```python
recon_loss = MSE(predicted_position, true_position)
           = mean((pred_x - true_x)² + (pred_y - true_y)²)

var_loss   = EmbeddingVarianceLoss(embeddings, batch)
           # Penalizes per-dimension std falling below target (VICReg-style)

total_loss = recon_loss + 0.25 * var_loss
```

**Why combined loss:**
- **MSE**: Position is continuous; penalizes large errors; smooth gradient
- **Variance regularizer**: Prevents "embedding collapse" where all 22 player embeddings converge to the same vector (a common failure mode in fully-connected GNNs). Forces each dimension to maintain a minimum standard deviation across nodes.
...
| Variance loss weight | 0.25 | Prevents embedding collapse without dominating reconstruction |
