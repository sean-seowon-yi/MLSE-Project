...
from src.graph_assembly import GraphAssembler, save_graphs, load_graphs
from src.dataset import train_val_test_split, create_data_loaders
from src.model import PlayerSimilarityAutoencoder
from src.train import Trainer, compute_reconstruction_metrics
...
def run_inference(config: Config, model_path: str, graphs_path: str) -> str:
    ...
    import numpy as np
    embeddings_matrix = np.stack(profiles['embedding'].values)
    emb_std = embeddings_matrix.std(axis=0).mean()
    emb_mean_norm = np.linalg.norm(embeddings_matrix, axis=1).mean()
    print(f"\n--- Embedding Health Check ---")
    print(f"  Mean per-dim std across players: {emb_std:.6f}")
    print(f"  Mean L2 norm: {emb_mean_norm:.4f}")
    if emb_std < 0.01:
        print("  WARNING: Embeddings may be collapsed (very low variance).")
    else:
        print("  Embeddings look diverse (healthy).")
    ...
    triu_idx = np.triu_indices(len(similarity_matrix), k=1)
    off_diag = similarity_matrix[triu_idx]
    print(f"  Similarity stats: mean={off_diag.mean():.4f}, "
          f"std={off_diag.std():.4f}, min={off_diag.min():.4f}, max={off_diag.max():.4f}")
    if off_diag.std() < 0.01:
        print("  WARNING: Similarities are nearly uniform — model may need more training.")
